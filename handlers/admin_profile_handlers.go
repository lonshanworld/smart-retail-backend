...
func HandleUpdateAdminProfile(c *fiber.Ctx) error {
	...
	query := fmt.Sprintf("UPDATE users SET %s WHERE id = $%d AND role = 'admin' RETURNING id, name, email, role, is_active, phone, assigned_shop_id, merchant_id, created_at, updated_at", strings.Join(setParts, ", "), argId)
